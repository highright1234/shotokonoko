...
fun newPlayerArrayList(vararg players: ProxiedPlayer): MutableList<ProxiedPlayer> {
    return Collections.synchronizedList(players.toMutableList()) // ArrayList
        .also { PlayerGCProcessor.addTarget(it) }
}

fun newPlayerHashSet(vararg players: ProxiedPlayer): MutableSet<ProxiedPlayer> {
    return players.associateWith { true } // LinkedHashSet
        .let(Collections::newSetFromMap)
        .also { PlayerGCProcessor.addTarget(it) }
}

fun newPlayerLinkedDeque(vararg players: ProxiedPlayer): Deque<ProxiedPlayer> {
